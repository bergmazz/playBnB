...
export const newSpotThunk = (spotData) => async (dispatch) => {
  const { address, city, state, country, name, description, price, images } =
    spotData;
...
export const editSpotThunk = (spot) => async (dispatch) => {
  const { address, city, state, country, name, description, price, spotId } =
    spot;
...
const spotReducer = (state = initialState, action) => {
  ...
  switch (action.type) {
    case POPULATE_SPOTS:
      // newState = {}
      Object.values(action.payload.Spots).forEach(
